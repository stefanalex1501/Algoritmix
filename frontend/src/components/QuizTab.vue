...
const TEXT_QUESTIONS = [
  ...
  // Heap Sort
  { prompt: "Care este complexitatea Heap Sort în orice caz (cel mai bun, mediu, cel mai rău)?", options: ["O(n log n)", "O(n²)", "O(n)", "O(log n)"], correct: "O(n log n)", explanation: "Heap Sort are O(n log n) garantat în toate cazurile. Construirea heap-ului = O(n), iar extragerea de n ori a maximului = O(n log n)." },
  { prompt: "Heap Sort este un algoritm stabil?", options: ["Nu, nu este stabil", "Da, este stabil", "Depinde de implementare", "Numai pentru n par"], correct: "Nu, nu este stabil", explanation: "Heap Sort nu este stabil: operațiile heapify pot reordona elemente egale relativ unele față de altele în timpul construcției sau extracției." },
  { prompt: "Ce structură stă la baza algoritmului Heap Sort?", options: ["Heap maxim (Max-Heap)", "Stivă (Stack)", "Coadă (Queue)", "Arbore AVL"], correct: "Heap maxim (Max-Heap)", explanation: "Heap Sort construiește un max-heap (v[1] = maximul), apoi extrage repetat maximul și îl plasează la finalul vectorului, reducând dimensiunea heap-ului." },

  // Counting Sort
  { prompt: "Care este complexitatea Counting Sort pentru n elemente cu valori în [0, k]?", options: ["O(n + k)", "O(n log n)", "O(n²)", "O(k log k)"], correct: "O(n + k)", explanation: "Counting Sort parcurge vectorul (O(n)) și tabloul de frecvențe (O(k)) → total O(n + k). Bate O(n log n) când k este mic." },
  { prompt: "Counting Sort este mai eficient decât Quick Sort când:", options: ["Valorile sunt întregi într-un interval mic (k << n)", "Vectorul are milioane de elemente cu valori mari", "Vectorul este aproape sortat", "Elementele sunt numere reale"], correct: "Valorile sunt întregi într-un interval mic (k << n)", explanation: "Dacă k (valoarea maximă) este mic față de n, O(n+k) bate clar O(n log n). Ex: sortarea notelor 1-10 ale n=10.000 studenți." },
  { prompt: "Counting Sort este un algoritm stabil?", options: ["Da, este stabil", "Nu, nu este stabil", "Depinde de vectorul de intrare", "Numai pentru valori distincte"], correct: "Da, este stabil", explanation: "Implementarea corectă (cu tablou cumulativ) plasează elementele egale în ordinea originală → Counting Sort este stabil." },

  // Algoritmul lui Euclid
  { prompt: "Ce calculează algoritmul lui Euclid?", options: ["Cel mai mare divizor comun (CMMDC)", "Cel mai mic multiplu comun (CMMMC)", "Suma cifrelor unui număr", "Numărul de divizori ai lui n"], correct: "Cel mai mare divizor comun (CMMDC)", explanation: "Algoritmul lui Euclid calculează CMMDC(a, b) prin împărțiri repetate: r = a mod b, a ← b, b ← r, până când b = 0." },
  { prompt: "CMMMC(a, b) se calculează din CMMDC(a, b) prin formula:", options: ["a / CMMDC(a,b) * b", "a * b + CMMDC(a,b)", "a + b - CMMDC(a,b)", "a * b * CMMDC(a,b)"], correct: "a / CMMDC(a,b) * b", explanation: "CMMMC(a, b) = a × b / CMMDC(a, b). Se împarte mai întâi (a / CMMDC, rezultat întreg) și abia apoi înmulțim cu b, pentru a evita overflow." },

  // Verificare număr prim
  { prompt: "De ce verificăm primalitatea unui număr n testând divizorii doar până la √n?", options: ["Dacă d > √n divide n, atunci n/d < √n și ar fi fost deja detectat", "Numerele prime sunt întotdeauna mai mici decât √n", "Divizorii mai mari decât √n nu există niciodată", "Este o convenție arbitrară pentru viteză"], correct: "Dacă d > √n divide n, atunci n/d < √n și ar fi fost deja detectat", explanation: "Dacă n = a × b cu a ≤ b, atunci a ≤ √n. Orice factor mai mare decât √n are perechea sa mai mică, deja testată." },
  { prompt: "Care este complexitatea verificării dacă un număr n este prim?", options: ["O(√n)", "O(n)", "O(log n)", "O(n²)"], correct: "O(√n)", explanation: "Testăm toți divizorii d de la 2 la √n → cel mult √n − 1 iterații. Dacă niciun d nu divide n, numărul este prim." },

  // Ciurul lui Eratostene
  { prompt: "De la ce valoare marcăm multiplii unui număr prim p în Ciurul lui Eratostene?", options: ["De la p²", "De la 2p", "De la p+1", "De la p"], correct: "De la p²", explanation: "Multiplii lui p mai mici decât p² (2p, 3p, …, (p-1)p) au un factor prim < p și au fost deja marcați anterior. Pornim de la p² pentru eficiență." },
  { prompt: "Ciurul lui Eratostene este preferat față de verificarea individuală a primalității când:", options: ["Avem nevoie de TOATE primele până la n", "Verificăm un singur număr mare", "n este mai mic decât 100", "Lucrăm cu numere prime foarte mari"], correct: "Avem nevoie de TOATE primele până la n", explanation: "Ciurul găsește toate primele până la n în O(n log log n), mult mai eficient decât a verifica individual fiecare număr în O(√n)." },
];
